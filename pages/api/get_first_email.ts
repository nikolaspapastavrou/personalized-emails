...
export async function getReply(sourceProductDescription: string, leadCompanyUrl: string, leadCompanyName: string, leadCompanyOperatorName: string, emailConversation: string[], sourceMeetingLink: string): Promise<any> {
  console.log(sourceProductDescription);
  console.log(leadCompanyUrl);
  ...
  const replyBodyPrompt = await promptUtils.get_reply_body(emailConversation, sourceProductDescription, sourceMeetingLink, leadCompanyName, leadCompanyOperatorName, companyInfo);
  console.log('Got body');
  console.log(replyBodyPrompt);
  ...
  // Return response
  console.log('Returning response');
  return { emailBody: emailTextBody.text };
};
